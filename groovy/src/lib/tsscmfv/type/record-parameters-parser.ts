...
export class RecordParametersParser extends BySingleCharTokenParser {
	private static Selector: ParserSelector;

	static initSelector(parsers: ParserSelectorArgs['parsers']) {
		if (RecordParametersParser.Selector != null) {
			throw new Error('RecordParametersParser.Selector is initialized.');
		}
		RecordParametersParser.Selector = new ParserSelector({
	...
	protected startBlock(context: ParseContext): void {
		const params = new BlockToken(T.RecordParamsBlk, this.createToken(context));
		context.sink(params);
		context.forward(1);
	}

	protected getInitBlockParserSelector(): ParserSelector {
		return RecordParametersParser.Selector;
	}
	...
	/**
	 * - when meet rparen or comma, end previous {@link T.RecordParamSeg} (if exists),
	 * - create a {@link T.RecordParamSeg} block when current block is not {@link T.RecordParamSeg}.
	 */
	protected beforeChildParsed(context: ParseContext, parser: TokenParser) {
		if (parser === RParenParserInstance || parser === CommaParserInstance) {
			...
			if (block.id === T.RecordParamSeg) {
				context.rise();
			}
		} else {
			const block = context.block();
			if (block.id !== T.RecordParamSeg) {
				const decl = new BlockToken(T.RecordParamSeg);
				context.sink(decl);
			}
		}
	}
	...
	static readonly instance = new RecordParametersParser();
}
